...
// Grid from the shared `pickerGrid`, on the DENSE ladder: model names are
// short, so more columns fit than the brand screen allows. Models come from the
// API (getModelsByBrand / getModelsBySeries) — nothing static.
        ...
        >
          {allowOther ? (
            <Pressable
              ...
              className="flex-row items-center rounded-2xl px-3.5 py-3 mb-4 active:opacity-80"
              style={{ borderWidth: 1, borderStyle: 'dashed', borderColor: ACCENT }}
            >
              <View
                className="h-9 w-9 rounded-lg items-center justify-center mr-3"
                style={{ backgroundColor: 'rgba(0,76,64,0.10)' }}
              >
                <Plus size={18} color={ACCENT} />
              </View>
              <View className="flex-1">
                <Text className="text-[13px] font-medium" style={{ color: ACCENT }}>Other — type the model</Text>
