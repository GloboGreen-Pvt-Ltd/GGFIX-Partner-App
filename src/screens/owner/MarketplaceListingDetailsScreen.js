...
function statusMeta(rawStatus) {
  ...
  if (s === 'SOLD' || s === 'COMPLETED') return { label: 'Selling Completed', icon: 'checkmark', bg: '#004C40', tint: '#E6F7E3', border: '#C8EEBF' };
  ...
  if (loading) {
    return (
      <View className="flex-1 bg-background">
        ...
        <View className="flex-1 items-center justify-center"><ActivityIndicator color="#004C40" /></View>
      </View>
    );
  }
  if (!item) {
                ...
                {allPhotos.map((url, i) => (
                  <View key={i} className="px-1 mb-3" style={{ width: '33.333%' }}>
                    <View
                      ...
                      style={{ height: 96, borderWidth: 1.5, borderStyle: 'dashed', borderColor: '#7FB8AE' }}
                    >
