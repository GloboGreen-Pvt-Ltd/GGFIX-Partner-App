...
export default function OwnerSellChooseSalesCategoryScreen({ navigation, route }) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#EFF5EE' }} edges={['top']}>
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 14 }}>
          ...
          >
            <Ionicons name="arrow-back" size={20} color="#087A0A" />
          </Pressable>
          <View style={{ flex: 1 }}>
        ...
        >
          ...
          >
            <Ionicons name="flash" size={16} color="#087A0A" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ color: '#087A0A', fontSize: 14, fontWeight: '800' }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingTop: 18 }}>
          ...
          >
            ...
            >
              {modelImageUrl ? (
                ...
                <DeviceIcon size={26} color="#087A0A" />
              )}
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 10, fontWeight: '800', letterSpacing: 0.5, color: '#087A0A', textTransform: 'uppercase' }}>
                Selected
              </Text>
              ...
              {categoryName ? (
                ...
                >
                  <Ionicons name="phone-portrait" size={11} color="#087A0A" />
                  <Text style={{ fontSize: 10, fontWeight: '700', color: '#087A0A', marginLeft: 4 }}>
                    {categoryName}
                  </Text>
                </View>
              ) : null}
        ...
        >
          ...
          {[
            { icon: 'shield-checkmark', color: '#087A0A', title: 'Verified buyers in your area', sub: 'We connect you with trusted, local buyers' },
            { icon: 'ribbon', color: '#F59E0B', title: 'Get paid quickly & safely', sub: 'Secure payments with instant transfers' },
            { icon: 'rocket', color: '#16BB05', title: 'Listing live in under a minute', sub: 'A few simple steps and you are done' },
          ].map((row, i) => (
            <View
              key={row.icon}
              style={{
